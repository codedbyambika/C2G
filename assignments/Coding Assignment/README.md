# Coding Assignment - Submission Report
 
## Student Profile
- **Name:** Ambika Bindal
 
## Progress Summary
- **Status:** 🟢 Completed
- **Submitted Questions:** 6 / 6
- **Total Score:** 32 / 34 points
- **Last Updated:** 6/16/2026, 1:38:06 PM (IST)
 
---
 
## Assignment Questions
| # | Question Title | Difficulty | Score | Status | Code Link |
|---|----------------|------------|-------|--------|-----------|
| 1 | Phone Charging | easy | 4 / 4 | 🟢 Passed | [View Code](./Question%201/question.java) |
| 2 | Weekend Discount | medium | 4 / 4 | 🟢 Passed | [View Code](./Question%202/question.java) |
| 3 | Smart Fan Controller | medium | 10 / 10 | 🟢 Passed | [View Code](./Question%203/question.java) |
| 4 | Internet Speed Checker | medium | 4 / 6 | 🟡 Partially Passed | [View Code](./Question%204/question.java) |
| 5 | Traffic Light Assistant | medium | 4 / 4 | 🟢 Passed | [View Code](./Question%205/question.java) |
| 6 | Mobile Data Rollover | hard | 6 / 6 | 🟢 Passed | [View Code](./Question%206/question.java) |

---
 
## Detailed Test Case Results

### Question 1: Phone Charging
- **Score:** 4 / 4 points
- **Visible Test Cases:** 2 / 2 passed
- **Hidden Test Cases:** 2 / 2 passed

### Question 2: Weekend Discount
- **Score:** 4 / 4 points
- **Visible Test Cases:** 2 / 2 passed
- **Hidden Test Cases:** 2 / 2 passed

### Question 3: Smart Fan Controller
- **Score:** 10 / 10 points
- **Visible Test Cases:** 2 / 2 passed
- **Hidden Test Cases:** 0 / 0 passed

### Question 4: Internet Speed Checker
- **Score:** 4 / 6 points
- **Visible Test Cases:** 2 / 2 passed
- **Hidden Test Cases:** 2 / 3 passed

### Question 5: Traffic Light Assistant
- **Score:** 4 / 4 points
- **Visible Test Cases:** 2 / 2 passed
- **Hidden Test Cases:** 2 / 2 passed

### Question 6: Mobile Data Rollover
- **Score:** 6 / 6 points
- **Visible Test Cases:** 2 / 2 passed
- **Hidden Test Cases:** 3 / 3 passed


*Generated automatically by Code-to-Grade (C2G) Assessment Platform.*